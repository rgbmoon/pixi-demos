...
import { type MockOptions, MockScenario } from '#src/mocks/types'
...
import type { WsTransportOptions } from '@pixi-demos/net/types'
...
export const createRound = ({
  ...
  // Транспорт биндится той же функцией bindNet, что и в композиции приложения: сценарий обменивается
  // сообщениями с моком через тот же код, что и игра. Опции сценария заменяют только таймауты транспорта
  bindNet(container, { url: WS_URL, ...transport })
  bindFsm(container)
  bindFlow(container)
