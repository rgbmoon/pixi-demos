...
**Статус: каркас.** Все пять слоёв целевой архитектуры реализованы, сквозной спин работает (кнопка → сеть → стор → контроллер → Spine): барабаны крутятся, тормозят и садятся на серверные символы, счёт ведётся по данным сервера (списание ставки, показ выигрыша, зачисление в кредит). Визуализации линий выплат и звука **пока нет** (см. [Целевая архитектура](#целевая-архитектура)).
