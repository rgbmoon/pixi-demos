...
**Стек:** React 19 · PIXI.js v8 · MobX 6 · Tailwind CSS v4 · react-router-dom v7 · TypeScript 6 · Vite 8; сеть — zod + partysocket, моки — MSW. **React Compiler включён** (через `reactCompilerPreset()` — см. [vite.config.ts](vite.config.ts)). `clsx` и `mobx-utils` в зависимостях, но пока не используются.
...
- Сергей при ревью может удалять эти комментарии — это ожидаемо, не возвращай их.
...
src/
  ...
  app/
    ...
    game-root.ts           # композиционный корень игры: Application + контроллеры + автомат (класс, без синглтона)
  api/                     # сетевой слой: service.ts (класс WsTransport + синглтон wsTransport) + types.ts + <name>-api.ts (эндпоинт + zod-DTO)
  components/              # переиспользуемые компоненты
  events/                  # эвентный слой: game-emitter.ts (класс + синглтон gameEmitter) + types.ts (карта GameEvents) + utils.ts (traceEvent) + helpers.ts (waitFor)
  ...
  constants/               # глобальные константы: environment.ts (WS_URL ← VITE_WS_URL), bg-blobs.ts
...
**Раскладка файлов в слое** — одинакова во всех слоях. Файл с классом содержит **только класс** (и его синглтон в конце); типы, константы и свободные функции выносятся в соседние файлы слоя:

- **`<name>.ts`** — класс и ничего кроме: ни свободных функций, ни типов, ни констант (`game-emitter.ts` → `GameEmitter` + `gameEmitter`).
...
- **`utils.ts`** — утилиты, которые вызывает **сам слой** (`events/utils.ts` → `traceEvent`, передаётся в конструктор `gameEmitter`); **`helpers.ts`** — утилиты **поверх** класса для внешних потребителей (`events/helpers.ts` → `waitFor`, его вызывают фазы автомата). Критерий один — кто вызывает.
...
- **`EnvelopeSchema`** в [service.ts](src/api/service.ts) — формат конверта читается рядом с кодом, который по нему разбирает сообщения.
...
- Транспорт — класс `WsTransport { request/subscribe/disconnect }`, состояние в приватных полях (фабрик-замыканий не пишем). Наружу — только синглтон `wsTransport`, сам класс не экспортируется. Потребители вызывают **методы синглтона**; свободных обёрток (`export const { request } = ...`) не выносим — деструктуризация методов теряет привязку `this`.
...
- Эндпоинт = тонкая функция в `<name>-api.ts`: вызывает транспорт, отдаёт распарсенный zod-схемой DTO (см. `sendSpin` в [root-api.ts](src/api/root-api.ts)).
...
- **`request` отменяем**: четвёртый аргумент `{ signal }` — тот же `AbortSignal`, что фаза передаёт в анимации (`sendSpin(bet, signal)`).

**Стейт (MobX)** — в `stores/`: пока один [spin-store.ts](src/stores/spin-store.ts) (состояние раунда). Паттерн стора: класс-синглтон с `makeObservable(this, { ... })` в конструкторе. В `stores/` — только MobX-классы; классы без реактивности размещаются в других слоях.

- MobX — для **игрового состояния**. UI-состояние интерфейса — на стороне React (`useState`, позже Zustand), не в MobX.
- **Аннотации только явные.** `makeAutoObservable` **не используем** — вместо него `makeObservable(this, { ... })` с перечислением **каждого** члена: `observable` / `observable.ref` / `computed` / `action` / `flow` / `false`. Что не перечислено — не наблюдается: тяжёлый объект не станет наблюдаемым непреднамеренно. Приватные члены требуют параметра типа: `makeObservable<this, 'dispose'>(...)` (см. [AsyncStream](src/stores/utils/async-stream.ts)).
...
- **Поле-`flow(...)`** (как `run` в [AsyncValue](src/stores/utils/async-value.ts)) аннотируем `false`: обёртка уже сделана вызовом `flow()`. Именно **поле**, а не generator-метод с аннотацией `flow`: только форма-поле типизируется как «возвращает промис» для `await` в фазах.
...
**Композиционный корень игры** — [app/game-root.ts](src/app/game-root.ts): владеет PIXI-`Application`, создаёт контроллеры, собирает автомат, держит `layout` на ресайз и уничтожает всё в `unmount()`.

- Лежит в `app/`, **над** слоями: он единственный, кто зависит от всех сразу.
- **Класс без синглтона**: инстанс создаёт страница в `useEffect` (`new GameRoot()`), cleanup вызывает `unmount()`. Время жизни инстанса совпадает с временем жизни роута.

**Событийная модель** — [events/](src/events/): `GameEmitter` (тонкая обёртка над `EventEmitter` из `pixi.js` — это eventemitter3, он уже в бандле), карта событий [types.ts](src/events/types.ts) и [waitFor](src/events/helpers.ts) (ожидание события промисом). Синглтон `gameEmitter` слои получают через DI (конструктор/контекст), а не импортом.
...
- Всё, что нужно фазе, приходит через `PhaseContext` (DI), включая `signal`. `fsm.dispose()` абортит его — все незавершённые `waitFor` и анимации реджектятся, при уходе со страницы все фазы завершаются.
- Сеть и анимация запускаются **параллельно** (`Promise.all([sendSpin(bet), reels.spin(signal)])`) — анимация не ждёт ответа сервера.
...
1. **Сторы (MobX)** — игровое состояние. Держим **независимые синглтоны**; RootStore/store-of-stores не вводим, координация между сторами — в фазах автомата. _(есть)_
...
- Не используй `makeAutoObservable` — только `makeObservable` с явными аннотациями.
