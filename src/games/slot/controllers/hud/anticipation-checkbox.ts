...
export class AnticipationCheckboxController extends LiveContainer {
  ...
  constructor(
    ...
    this.checkbox = new Checkbox({
      label: 'FORCE ANTICIPATION',
      width: PANEL_ROW_WIDTH,
      onTap: () => {
        slotStore.toggleAnticipationForced()
    ...
    this.watch(
      () => slotStore.isAnticipationForced,
      (isAnticipationForced) => this.checkbox.setChecked(isAnticipationForced),
      { fireImmediately: true }
    )

    this.watch(
      () => slotStore.canToggleAnticipationForced,
