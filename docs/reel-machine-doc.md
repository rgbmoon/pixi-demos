...
**Виды движения барабана выносятся в отдельные классы.** Сейчас `Reel.advance` ветвится по фазе:
прокрутка или посадка. Каскад добавляет третий вид — падение символов, и он устроен по-другому:
каждый слот падает на своё расстояние, поэтому позиции слотов уже не выводятся из одного `offset`
барабана. На каскадах движение выносится за интерфейс (условно `ReelMotion` с `advance(deltaFrames)`
и признаком завершения): прокрутка, посадка и падение становятся его реализациями, `Reel` держит
текущее движение. Held, респин и Hold & Win нового движения не добавляют — удержанный барабан
пропускается в `spin()`, поэтому интерфейс вводится вместе с каскадами.
